# This file is from Trio, somewhat modified.
...
from .abc_streams import AbstractStream # , Listener
...
class ConflictDetector:
    ...
    def __enter__(self):
        if self._held:
            raise BlockingIOError(self._msg)  # TODO invent a better error
        else:
            self._held = True
...
class SSLStream(AbstractStream):
    ...
    def __init__(
        ...
        *,
        ...
        self._inner_send_lock = asyncio.Lock()  # must be FIFO
        self._inner_recv_count = 0
        self._inner_recv_lock = asyncio.Lock()
    ...
    async def aclose(self):
        ...
        try:
            ...
            except (BrokenPipeError, trio.BusyResourceError):
                pass
        except BaseException:
            # Failure! Kill the stream and move on.
            self.transport_stream._sync_close()
            raise
        else:
